...
class Config():
    MODELS = {
        ...
        'CLER': LanguageModelConfig(BertForSequenceClassification, BertConfig, 'roberta-base', BertTokenizer),
    }

    DATASETS = {
        'synthetic_companies': os.path.join('synthetic_data', 'seed_0', 'synthetic_companies_dataset_seed_0_size_868254_sorted.csv'), # Uses the full dataset (i.e. the train/val/test splits)
        'synthetic_companies_small': os.path.join('synthetic_data', 'seed_0', 'synthetic_companies_dataset_seed_0_size_868254_sorted.csv'), # Uses the manually labeled splits (filtered_train/filtered_val/test splits)
        'wdc': os.path.join('wdc_80pair', 'wdc_80pair.csv'),
        'camera': os.path.join('camera', 'camera.csv'),
        'monitor': os.path.join('monitor', 'monitor.csv'),
        'musicbrainz': os.path.join('musicbrainz', 'musicbrainz.csv'),
    }

    SPLITS = {'synthetic_companies': os.path.join('synthetic_data', 'seed_0', 'companies'),
              'wdc': 'wdc_80pair',
              'camera': 'camera',
              'monitor': 'monitor',
...
def read_arguments_train():
    ...
    parser.add_argument('--dataset_name', default='synthetic_companies_small',
    ...
    if not args.experiment_name:
        ...
        if 'companies' in args.dataset_name:
        ...
        if os.path.isfile(experiment_config_path(args.experiment_name)):
            ...
            # compatability for old models before
...
def update_args_with_config(experiment_name: str, args: argparse.Namespace = argparse.Namespace()) -> argparse.Namespace:
    ...
    elif "CLER" in experiment_name:
        args.seed = 0
        args.dataset_name = args.matcher
        args.model_name = 'CLER'
    else:
