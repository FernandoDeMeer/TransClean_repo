...
class PyTorchModel:
    def __init__(self, args):
        self.args = args
        self.dataset = ExperimentDataset.create_instance(self.args.dataset_name, self.args.model_name,
                                                      seed=self.args.seed,
    ...
    def test(self, epoch: int = 0, global_step: int = 0, finetune=False):
        self._reset_prediction_buffer()
        ...
        if finetune:
            self.save_test_predictions(epoch, finetune=True)
            print("Epoch {}, Test Loss: {}, Test Accuracy: {}".format(epoch, test_loss, test_acc))
            if hasattr(self, 'finetune_dict'):
                self.finetune_dict['test_loss'].append(test_loss)
                self.finetune_dict['test_acc'].append(test_acc)
            else:
                self.finetune_dict = {
                    'test_loss': [test_loss],
                    'test_acc': [test_acc]
    ...
    def fine_tune(self, epochs, finetune_results_path, finetuning_iteration, test_after_finetune=False):
        global_step = 0
        total_loss, prev_epoch_loss, prev_loss = 0.0, 0.0, 0.0

        for epoch in tqdm(range(1, epochs + 1), desc=f'Fine-tuning for {epochs} epochs ...'):
            sample_count, sample_correct = 0, 0

            for step, batch_tuple in tqdm(enumerate(self.train_data_loader),
                                          desc=f'[FINE-TUNING] Running epoch {epoch}/{epochs} ...',
                                          total=len(self.train_data_loader)):
                self.network.train()
                outputs, inputs, _ = self.predict(batch_tuple)
        ...
        file_name = '{}_finetune_iteration_{}.pt'.format(self.args.model_name, finetuning_iteration)
        model_path = os.path.join(finetune_results_path, file_name)
        torch.save(self.network.state_dict(), model_path)

        if test_after_finetune:
            prediction_buffer = self.finetune_test(epoch)

    def finetune_test(self, epoch: int= 0):
        self._reset_prediction_buffer()
        total_loss, prev_loss = 0.0, 0.0
        sample_count, sample_correct = 0, 0
    ...
    def evaluate_dataloader(self, data_loader, leave_tqdm = True):
        prediction_buffer = self.get_new_prediction_buffer(data_loader)

        for step, batch_tuple in tqdm(enumerate(data_loader), desc='[EVALUATING]',
                                      total=len(data_loader), leave=leave_tqdm):
            ...
            prediction_buffer = self.log_predictions_to_buffer(prediction_buffer, data_loader, raw_inputs, predictions, prediction_proba, step=step)
    ...
    def log_predictions_to_buffer(self, pred_buffer, data_loader, raw_inputs, predictions, prediction_proba, step=0):

        batch_size = data_loader.batch_size
        num_samples = len(data_loader.dataset.idx_df) 

        start_idx = step * batch_size
        end_idx = np.min([((step + 1) * batch_size), num_samples])

        # Save the IDs, labels and predictions in the buffer
        pred_buffer['lids'][start_idx:end_idx] = tensor_to_list(raw_inputs['lids'])
        pred_buffer['rids'][start_idx:end_idx] = tensor_to_list(raw_inputs['rids'])
        pred_buffer['labels'][start_idx:end_idx] = tensor_to_list(raw_inputs['labels'])
        pred_buffer['predictions'][start_idx:end_idx] = tensor_to_list(predictions)
        pred_buffer['prediction_proba'][start_idx:end_idx] = tensor_to_list(prediction_proba)
    ...
    #
    def save_test_predictions(self, epoch, finetune=False):
        if finetune:
            file_name = "".join([self.args.model_name, '__finetune_prediction_log__ep', str(epoch), '.csv'])
        else:
    ...
    def get_new_prediction_buffer(self, data_loader):

        num_test_samples = len(data_loader.dataset.idx_df)
        prediction_buffer = {
            'lids': np.zeros(num_test_samples, dtype=np.int),
            'rids': np.zeros(num_test_samples, dtype=np.int),
            'labels': np.zeros(num_test_samples, dtype=np.int),
