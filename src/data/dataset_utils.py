...
def add_random_nonmatches(df: pd.DataFrame, dataset_type: str, match_matrix: csr_matrix, nonmatch_ratio, name):
    ...
    #
    # Currently, we adjust the :match_matrix after each pass, so that the matrix
...
def add_companies_nonmatches_w_hard_neg_pairs(train_given, test_given, val_given, nonmatches, nonmatch_ratio, name):
    # We first add the hard pairs to the train and test matches in a proportional way
    lens = [len(train_given), len(test_given), len(val_given)]
    ratios = np.array(lens) / sum(lens)

    # This assumes the full data is in order [train, test, valid], with an optional validation
    # set that might be empty. train/test/val sets are thus:
    #   - 0 TO train_end_idx
    #   - train_end_idx TO test_end_idx
    #   - and test_end_idx TO end
    train_end_idx = int(ratios[0] * len(nonmatches))
    # if val_given is empty the the cut off should be the end of the list
    # train_end_idx + int(ratios[1] * len(nonmatches)): this part would yield len(nonmatches) - 1 if val_given is empty
    # hence we would be missing one sample
    test_end_idx = train_end_idx + int(ratios[1] * len(nonmatches)) if ratios[-1] != 0 else len(nonmatches)


    train_given = pd.concat((train_given, nonmatches.iloc[:train_end_idx]), axis=0)
    test_given = pd.concat((test_given, nonmatches.iloc[train_end_idx:test_end_idx]), axis=0)
    # this will be empty if val_given was empty to begin with
    val_given = pd.concat((val_given, nonmatches.iloc[test_end_idx:]), axis=0)

    # Finally we add randomly sampled non-matches
    test_given, train_given, val_given = add_random_non_matches_train_val_test(train_given, test_given, val_given,
                                                                    nonmatch_ratio=nonmatch_ratio, name=name)

    return test_given, train_given, val_given

def add_companies_nonmatches(train_given, test_given, val_given, nonmatch_ratio, name):
    # We add randomly sampled non-matches
    test_given, train_given, val_given = add_random_non_matches_train_val_test(train_given, test_given, val_given,
                                                                    nonmatch_ratio=nonmatch_ratio, name=name)
    return test_given, train_given, val_given
