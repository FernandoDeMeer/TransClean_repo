...
def update_pseudoBK_df(pseudo_BK_df, blocker_pseudo_pos, blocker_pseudo_neg):
    if len(blocker_pseudo_pos) == 0 and len(blocker_pseudo_neg) == 0:
        return pseudo_BK_df
    pseudo_BK_df = None
    try:
        ...
        pseudo_new = []
        if len(blocker_pseudo_pos) > 0:
            pseudo_new = np.array(blocker_pseudo_pos)
        if len(blocker_pseudo_neg) > 0:
            pseudo_new = np.array(blocker_pseudo_neg)
    ...
    if pseudo_BK_df is None:
        ...
        df_all = pd.concat([dforg, dfnew])
        df_all = pd.concat([df_all, dfboth])
    return df_all
...
def update_annotated_df(annotated_df, annotated_pos, annotated_neg):
    ...
    if annotated_df is None:
        ...
        annotated_df = pd.concat([annotated_df, new_annotated_df])
        return annotated_df
...
def MC_self_check(pseudo_pos_matcher, pseudo_neg_matcher, pos_score, neg_score):
    ...
    if len(pairs) > 0:
        ...
        df_pos = pd.concat([df_pos_single, df_pos])
        df_pos = df_pos.drop_duplicates()
        df = pd.concat([df_pos, df_neg])
    else:
        df = pd.concat([df_pos_single, df_neg])
...
def load_gt(hp):
    # In CLER's original implementation, the ground truth is stored in a file named 'matches.csv' in the dataset directory.
    # In other datasets, the ground truth pairs are distributed in train.csv, val.csv, and test.csv. We will use this function
    # during training only, so we only load train and val pairs.
    if 'companies' in hp.dataset:
        dataset_path = hp.path
        gt_df = pd.read_csv(os.path.join(dataset_path, 'filtered_train.csv'), index_col=0)
        gt_df = gt_df.rename(columns = {'lid': 'ltable_id', 'rid': 'rtable_id'})
        gt_df, _ = translate_ids(dataset_path, gt_df)
        gt = gt_df[['ltable_id', 'rtable_id', 'label']]
        ...
        gt_df_val = pd.read_csv(os.path.join(dataset_path, 'val.csv'))
        gt_df = pd.concat([gt_df, gt_df_val])
        gt_df = gt_df.rename(columns = {'lid': 'ltable_id', 'rid': 'rtable_id'})
        gt_df, _ = translate_ids(dataset_path, gt_df)
        gt = gt_df[['ltable_id', 'rtable_id', 'label']]
    else:
...
def get_SentEmb(EmbModel, dataset):
    ...
    emb = []
    with torch.no_grad():
        for batch in tqdm(data_iter, desc= 'Processing Embbedings', total=len(data_iter)):
            x1, x1_mask = batch 
...
def get_topK_sim(hp):
    if 'wdc' in hp.dataset:
        attr_listA, entity_listA = read_entity(hp.path, table='wdc_80pair', shuffle=False)
        attr_listB, entity_listB = attr_listA, entity_listA
    elif 'companies' in hp.dataset:
        attr_listA, entity_listA = read_entities_companies(hp.path)
        attr_listB, entity_listB = read_entities_companies(hp.path)
    elif 'camera' in hp.dataset:
        attr_listA, entity_listA = read_entity(hp.path, table='camera', shuffle=False)
        attr_listB, entity_listB = attr_listA, entity_listA
    elif 'monitor' in hp.dataset:
        attr_listA, entity_listA = read_entity(hp.path, table='monitor', shuffle=False)
        attr_listB, entity_listB = attr_listA, entity_listA
    elif 'musicbrainz' in hp.dataset:
        attr_listA, entity_listA = read_entity(hp.path, table='musicbrainz', shuffle=False)
        attr_listB, entity_listB = attr_listA, entity_listA
    else:
    ...
    from src.CLER.dataset import SingleEntityDataset
...
def load_attributes(path, load_test=False):
    if 'companies' in path:
        attr_list, entity_list = read_entities_companies(path, load_test)
        return attr_list, entity_list, attr_list, entity_list
    elif 'wdc' in path:
        attr_listA, entity_listA = read_entity(path, table='wdc_80pair', shuffle=False)
        if load_test:
            # Load the test indexes
            test_ids = pd.read_csv(os.path.join(path, 'test.csv'))
            test_ids = np.concatenate([test_ids['lid'].values, test_ids['rid'].values])
            # Filter the records
            entity_listA = pd.DataFrame(entity_listA)
            entity_listA.reset_index(inplace=True)
            entity_listA.rename(columns={"index":'id'}, inplace=True)
            entity_listA = entity_listA[entity_listA['id'].isin(test_ids)]
            entity_listA.set_index('id')
            entity_listA = entity_listA.drop(columns=['id'])
            entity_listA = entity_listA.values
        return attr_listA, entity_listA, attr_listA, entity_listA
    elif 'camera' in path:
        # Check if the dataset_path has the file already added
        if '.csv' in path:
            path = os.path.dirname(path)
            attr_listA, entity_listA = read_entity(path, table='camera', shuffle=False)            
        else:
            attr_listA, entity_listA = read_entity(path, table='camera', shuffle=False)
        ...
        if '.csv' in path:
            path = os.path.dirname(path)
            attr_listA, entity_listA = read_entity(path, table='monitor', shuffle=False)
        else:
            attr_listA, entity_listA = read_entity(path, table='monitor', shuffle=False)
        ...
        if load_test:
            ...
            entity_listA = entity_listA.values
        return attr_listA, entity_listA, attr_listA, entity_listA
    elif 'musicbrainz' in path:
        # Check if the dataset_path has the file already added
        if '.csv' in path:
            path = os.path.dirname(path)
            attr_listA, entity_listA = read_entity(path, table='musicbrainz', shuffle=False)
        else:
            attr_listA, entity_listA = read_entity(path, table='musicbrainz', shuffle=False)
...
def read_entity(dataset, table=None, shuffle=True):
    ...
    if table is None:
        ...
        if 'wdc' not in dataset:
            df = pd.concat([df,pd.read_csv(os.path.join(dataset, 'tableB.csv'), sep = ',', index_col=0)])
    else:
...
def read_entities_companies(dataset_path, load_test=False):
    """
    Read entities from the synthetic companies dataset. We load the filtered_train and test files in order to filter the records we need.
    """
    df = load_companies_df(dataset_path, load_test)
    entity_list = df.values
    return list(df.columns), list(entity_list)

def load_companies_df(dataset_path, load_test=False):
    # Check if the dataset_path has the file already added
    if '.csv' not in dataset_path:
        if 'companies' in dataset_path:
            # Remove the 'companies' folder from the path
            dataset_path = os.path.dirname(dataset_path)
        df = pd.read_csv(os.path.join(dataset_path, 'synthetic_companies_dataset_seed_0_size_868254_sorted.csv'), sep = ',', low_memory=False)
    else:
        df = pd.read_csv(dataset_path, sep = ',', low_memory=False)
        dataset_path = os.path.dirname(dataset_path)
    # Filter the records
    if load_test:
        test_ids = pd.read_csv(os.path.join(dataset_path, 'companies', 'test.csv'), sep = ',', index_col=0)
        test_ids = np.concatenate([test_ids['lid'].values, test_ids['rid'].values])

        df.reset_index(inplace=True)
        df.rename(columns={"index":'id'}, inplace=True)
        df = df[df['id'].isin(test_ids)] # We filter the records involved in the test set
    else:
        filtered_train_ids = pd.read_csv(os.path.join(dataset_path, 'companies', 'filtered_train.csv'), sep = ',', index_col=0)
        filtered_train_ids = np.concatenate([filtered_train_ids['lid'].values, filtered_train_ids['rid'].values])

        df.reset_index(inplace=True)
        df.rename(columns={"index":'id'}, inplace=True)
        df = df[df['id'].isin(filtered_train_ids)] # We filter the records involved in the filtered training set

    df.set_index('id')
    df = df.drop(columns=['id', 'data_source_id', 'inserted', 'last_modified', 'external_id'])
    return df

def translate_ids(dataset_path, idxs_df, load_test=False):
    """
    The ids in some datasets reference those in the raw csvs.
    A lot of CLER funtions expect positional indices instead of the ids in the raw file, so we translate them.

    Args:
        df: The dataframe containing the original ids as the index.
        idxs_df: id with columns 'ltable_id/lid' and 'rtable_id/rid' containing the ids to be translated.
    """
    if 'companies' in dataset_path:
        df = load_companies_df(dataset_path, load_test)
    elif 'wdc' in dataset_path:
        df = pd.read_csv(os.path.join(dataset_path, 'wdc_80pair.csv'), sep = ',', index_col=0)
        if load_test:
            # Load the test indexes
            test_ids = pd.read_csv(os.path.join(dataset_path, 'test.csv'))
            test_ids = np.concatenate([test_ids['lid'].values, test_ids['rid'].values])
            # Filter the records
            df.reset_index(inplace=True)
            df.rename(columns={"index":'id'}, inplace=True)
    ...
    elif 'camera' in dataset_path:
        if '.csv' in dataset_path:
            dataset_path = os.path.dirname(dataset_path)
        df = pd.read_csv(os.path.join(dataset_path, 'camera.csv'), sep = ',', index_col=0)
        if load_test:
            # Load the test indexes
            test_ids = pd.read_csv(os.path.join(dataset_path, 'test.csv'))
            test_ids = np.concatenate([test_ids['lid'].values, test_ids['rid'].values])
            ...
            df.set_index('id')
            df = df.drop(columns=['id'])
    elif 'monitor' in dataset_path:
        if '.csv' in dataset_path:
            dataset_path = os.path.dirname(dataset_path)
        df = pd.read_csv(os.path.join(dataset_path, 'monitor.csv'), sep = ',', index_col=0)
        if load_test:
            # Load the test indexes
            test_ids = pd.read_csv(os.path.join(dataset_path, 'test.csv'))
            test_ids = np.concatenate([test_ids['lid'].values, test_ids['rid'].values])
            ...
            df.set_index('id')
            df = df.drop(columns=['id'])
    elif 'musicbrainz' in dataset_path:
        if '.csv' in dataset_path:
            dataset_path = os.path.dirname(dataset_path)
        df = pd.read_csv(os.path.join(dataset_path, 'musicbrainz.csv'), sep = ',', index_col=0)
        if load_test:
            # Load the test indexes
            test_ids = pd.read_csv(os.path.join(dataset_path, 'test.csv'))
            test_ids = np.concatenate([test_ids['lid'].values, test_ids['rid'].values])
    ...
    for idx, id in enumerate(df.index):
        id2idx[id] = idx
    # Translate the ids
    if 'lid' in idxs_df.columns and 'rid' in idxs_df.columns:
        idxs_df['lid'] = idxs_df['lid'].apply(lambda x: id2idx[x])
        idxs_df['rid'] = idxs_df['rid'].apply(lambda x: id2idx[x])
    elif 'ltable_id' in idxs_df.columns and 'rtable_id' in idxs_df.columns:
...
def translate_ids_back(df, test_ids_dict):
    """
    Translate the positional indices back to the original ids. 

    Args:

        df: The dataframe containing the positional indices as the index as tuples.
    
        test_ids_dict: The test_ids_dict is the dictionary whose keys are the original ids and values are the positional indices.
    """
    # Reverse the dictionary
    idx2id = {v: k for k, v in test_ids_dict.items()}
    # Translate the ids back (from positional indices in the index to the original ids)
    df['lid'] = ''
    df['rid'] = ''
    for idx in df.index:
        df.at[idx, 'lid'] = idx2id[idx[0]]
        df.at[idx, 'rid'] = idx2id[idx[1]]
...
def load_CLER_matcher(experiment_name):
    model = DittoConcatModel(lm = 'roberta')
    model = model.cuda()
    model.optimizer = AdamW(model.parameters(), lr=3e-5)
    model.load_state_dict(torch.load(os.path.join('models', experiment_name, 'last_matcher_model.pt')), strict= False)
    return model

def load_valid_df(hp):
    # 'valid0207.csv' is the validation set for the dataset, it consists of 500 randomly chosen pairs (CLER's paper: "500 pairs are randomly selected and annotated as validation data for all compared methods")
    path = hp.path

    if 'companies' in hp.dataset:
        # In the synthetic companies dataset, we only have positive pairs in the training set, so we choose a subset of the training pairs 
        # to create the validation set. We select 40 positive pairs and 460 negative pairs.

        valid_df = pd.read_csv(os.path.join(hp.path, 'filtered_train.csv'), index_col=0) # Load the whole set of positive training pairs
        valid_df = valid_df.rename(columns = {'lid': 'ltable_id', 'rid': 'rtable_id'})
        valid_df, _ = translate_ids(path, valid_df)
        pos_valid = valid_df[valid_df['label']==1].iloc[:40] # Select the first 40 positive pairs 
        # In order to get negative pairs, we select random 460 record pairs and check that they are not positive pairs
        all_ids = set(valid_df['ltable_id'].values).union(set(valid_df['rtable_id'].values))
        neg_valid = []
        random.seed(0) # Set the seed to make the results reproducible
        while len(neg_valid) < 460:
            l, r = random.sample(all_ids, 2)
            record_pair = valid_df[(valid_df['ltable_id']==l) & (valid_df['rtable_id']==r) | (valid_df['ltable_id']==r) & (valid_df['rtable_id']==l)]
            if len(record_pair) == 0:
                neg_valid.append([l, r, 0])
        valid_df = pd.concat([pos_valid, pd.DataFrame(neg_valid, columns=['ltable_id', 'rtable_id', 'label'])])

    elif 'wdc' in hp.dataset:
        valid_df = pd.read_csv(os.path.join(hp.path, 'train.csv'))
        valid_df_val = pd.read_csv(os.path.join(hp.path, 'val.csv'))
        valid_df = pd.concat([valid_df, valid_df_val])
        valid_df = valid_df.rename(columns = {'lid': 'ltable_id', 'rid': 'rtable_id'})
        valid_df, _ = translate_ids(path, valid_df)
        valid_df = valid_df.sample(n=500, random_state=0) # Select 500 random pairs from the train set

    elif 'camera' in hp.dataset or 'monitor' in hp.dataset or 'musicbrainz' in hp.dataset:
        # The camera/monitor/musicbrainz datasets only have positive pairs in the training set, so we add randomly selected negative pairs
        # We select 40 positive pairs and 460 negative pairs.
        valid_df = pd.read_csv(os.path.join(hp.path, 'train.csv'))
        valid_df_val = pd.read_csv(os.path.join(hp.path, 'val.csv'))
        valid_df = pd.concat([valid_df, valid_df_val])
        valid_df = valid_df.rename(columns = {'lid': 'ltable_id', 'rid': 'rtable_id'})
        valid_df, _ = translate_ids(path, valid_df)
        pos_valid = valid_df[valid_df['label']==1].iloc[:40]
        all_ids = set(valid_df['ltable_id'].values).union(set(valid_df['rtable_id'].values))
        neg_valid = []
        random.seed(0)
        while len(neg_valid) < 460:
...
def load_valid_data(hp, valid_df):

    if 'companies' in hp.dataset:
        # 'test_idxs.csv' is the set of indices of the records in the test set, in our case we have loaded train-only records, so we don't need to populate this variable
        test_idxs = np.array([])
        valid_data = valid_df[['ltable_id', 'rtable_id', 'label']].values
    elif 'wdc' in hp.dataset or 'camera' in hp.dataset or 'monitor' in hp.dataset or 'musicbrainz' in hp.dataset:
        # 'test_idxs.csv' is the set of indices of the records in the test set, in our case we have loaded train-only records, so we don't need to populate this variable
        test_idxs = np.array([])
        valid_data = valid_df[['ltable_id', 'rtable_id', 'label']].values
