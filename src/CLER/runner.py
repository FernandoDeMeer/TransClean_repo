...
from src.CLER.dataset import GTDatasetWithLabel, AugDataset, AugDatasetWithLabel, SingleEntityDataset, GTDatasetWithLabelWeights
# import matplotlib.pyplot as plt
from sklearn import metrics

def pred_scores(model, dataset):
    iterator = DataLoader(dataset=dataset, batch_size=32, collate_fn=dataset.pad)
    model.eval()
    scores_all = []
    for i, batch in tqdm(enumerate(iterator), desc='Prediction Scores', total=len(iterator)):
        x, _, y = batch   
        logits = model(x)
...
def eval_matcher(model, dataset, if_valid = False):
    ...
    for i, batch in tqdm(enumerate(iterator), desc='Evaluating Matcher', total=len(iterator)):
        x, _, y = batch   
        logits = model(x)
        try:
...
def train_matcher(model, train_set, optimizer, hp, scheduler=None):
    ...
    for batch in tqdm(iterator, desc= 'Training Matcher', total=len(iterator)):
        x, _, y, w = batch
        # forward
        optimizer.zero_grad()
        logits = model(x)
...
def train_blocker(blocker, train_set, bk_opt, hp, scheduler=None):
    ...
    w_list = []
    for batch in tqdm(iterator, desc='Training Blocker', total=len(iterator)):
...
def eval_blocker(blocker, blocker_valid):
    iterator = DataLoader(dataset=blocker_valid, batch_size=32, collate_fn=blocker_valid.pad)
    blocker.eval()
    scores, y_truth = [], []
    for i, batch in tqdm(enumerate(iterator), desc='Evaluating Blocker', total=len(iterator)):
...
def train(hp):
    ...
    model_path = os.path.join('models', hp.dataset + '_CLER_' + str(hp.total_budget) + '_seed_' + str(hp.run_id))
    ...
    train_data = np.concatenate([train_data, [[1]]*len(train_data)], 1) # 1 weights for pseudo labels
    ...
    '''
        Warm-up Stage
        generate pseudo labels for training and validation
    '''
    best_valid_F1 = -1
    if os.path.exists(os.path.join(model_path, 'matcher_model.pt')):
        ...
        while True:
            ...
            while True:
                ...
                if valid_F1 > best_valid_F1:
                    ...
                    if hp.save_model:
                        torch.save(model.state_dict(), os.path.join(model_path, 'matcher_model.pt'))
                elif epoch >= 10 or epoch >= 5 and (best_valid_F1 >= 0.5 or best_valid_F1 == 0 and train_loss_list[0] - np.mean(train_loss_list) <= 0.01):
                    break 
    ...
    '''
    valid_data, test_idxs = load_valid_data(hp=hp, valid_df=valid_df)
    ...
    while True:
        ...
        for epoch in range(1, epochs + 1):
            ...
            if (BK_valid_F1 > best_BK_valid_F1) or (flag == False and BK_valid_F1 == best_BK_valid_F1):
                ...
                if hp.save_model:
                    torch.save(blocker.state_dict(), os.path.join(model_path, 'blocker_model.pt'))   
                    # if hp.run_id==0 and hp.total_budget in [500, 2000]:
                    #     torch.save(blocker.state_dict(), os.path.join(model_path, 'Iter'+str(active_iteration)+'_blocker_model.pt'))
                flag = True
            if remaining_budget <= 0 and last_BK_F1 < BK_valid_F1:
                last_BK_F1 = BK_valid_F1
                torch.save(blocker.state_dict(), os.path.join(model_path, 'last_blocker_model.pt'))
        ...
        try:
            blocker.load_state_dict(torch.load(os.path.join(model_path, 'blocker_model.pt')))
        except:
            pass
        if flag:
            ...
            blocker_pseudo_pos, blocker_pseudo_neg = gen_pseudo_blocker(topkA, topkB, distA, distAll, valid_df)
            try:
                blocker_pseudo = np.concatenate([blocker_pseudo_pos, blocker_pseudo_neg], 0)
            except:
                blocker_pseudo = np.array([])
                if len(blocker_pseudo_pos) > 0:
                    blocker_pseudo = np.array(blocker_pseudo_pos)
                if len(blocker_pseudo_neg) > 0:
                    blocker_pseudo = np.array(blocker_pseudo_neg)
            pos_pre, pos_rec, neg_pre, neg_rec = evaluate_pseudo(blocker_pseudo, annotated_df, hp)
            pseudo_BK_df = update_pseudoBK_df(pseudo_BK_df, blocker_pseudo_pos, blocker_pseudo_neg)
        ...
        for epoch in range(1, epochs + 1):
            ...
            if remaining_budget <= 0 and last_MC_F1 < MC_valid_F1:
                torch.save(model.state_dict(), os.path.join(model_path, 'last_matcher_model.pt'))
            ...
            if MC_valid_F1 > best_MC_valid_F1 or (flag_matcher == False and MC_valid_F1 == best_MC_valid_F1):
                ...
                if hp.save_model:
                    torch.save(model.state_dict(), os.path.join(model_path, 'matcher_model.pt'))
                    # if hp.run_id==0 and hp.total_budget in [500, 2000]:
                    #     torch.save(model.state_dict(), os.path.join(model_path, 'Iter'+str(active_iteration)+'_matcher_model.pt'))
                early_stop = 0
